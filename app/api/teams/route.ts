...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'add_member_by_username') {
      if (!teamId || !userId || !usernameToAdd) {
        return new Response(
          JSON.stringify({
            error:
              'teamId, userId, and usernameToAdd are required',
          }),
          {
            status: 400,
      ...
      const normalized = String(usernameToAdd)
        .trim()
        .toLowerCase()
      ...
      const existing = (team.members || []).some(
        (m: any) => m.userId === targetUser.id
      )

      if (existing) {
        return new Response(
          JSON.stringify({ error: 'User already in group' }),
          {
            status: 409,
            headers: { 'Content-Type': 'application/json' },
          }
      ...
      await db.collection('teams').updateOne(
        {
          id: teamId,
          'members.userId': { $ne: targetUser.id },
        },
        {
          $push: {
            members: {
    ...
    if (!name || !userId) {
      return new Response(
        JSON.stringify({
          error: 'Team name and userId are required',
        }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
