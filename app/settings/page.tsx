...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const savedAutoThink = localStorage.getItem('autoThink')
    if (savedAutoThink !== null) setAutoThink(savedAutoThink === 'true')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('autoThink', autoThink.toString())
  ...
  }, [notifications, autoSave, autoThink, accentColor, session])
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[var(--bg-secondary)] border border-[var(--surface-border)] rounded-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-[var(--text-primary)]">Auto Thinking</p>
                        <p className="text-sm text-[var(--text-muted)]">
                          Enable thinking animation for complex questions
                        </p>
                      </div>
                      <button
                        onClick={() => setAutoThink(!autoThink)}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          autoThink ? 'bg-[var(--accent)]' : 'bg-[var(--surface-border)]'
                        }`}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                            autoThink ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
